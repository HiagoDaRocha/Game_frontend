...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let box = document.getElementById('challenge5'); // Seleciona o elemento com ID "challenge5"
    ...
    textarea.addEventListener('input', function() {
        ...
        // Verifica se o texto contém "img" ou "#challenge5" seguido de chaves
        let ValidFormat = styles.match(/^\s*(img|#challenge5)\s*\{\s*([^{}]*;\s*)+\s*}\s*$/);

        // Se o formato do texto é válido, aplica os estilos ao elemento com id "challenge5"
        if (ValidFormat) {
            
            // Extrai o conteúdo entre as chaves
            let cssContent = styles.replace(/^\s*(img|#challenge5)\s*\{\s*|\s*}\s*$/g, '');
